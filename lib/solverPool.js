...
class SolverPool {
        ...
        for (let i = 0; i < this.maxPoolSize; i++) {
            ...
            this.workers[i].worker.onmessage = (event) => {
                if (event.data.msgType === 'solution') {
                    ...
                    let shouldContinueTask = this.onSolution({
    ...
    setOnStart(startAction) {
        this.onStart = startAction;
    }

    setOnSolution(solutionAction) {
        this.onSolution = solutionAction;
    }
    ...
    beginNext() {
        ...
        const shouldStartTask = this.onStart({
            numbers,
            target
        });
        if (!shouldStartTask) {
